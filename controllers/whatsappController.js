...
const WABA_URL = `https://graph.facebook.com/v22.0/893382447192302/messages`;
const AUTH = {
  Authorization: `Bearer EAALnrroBkZCYBQGtBCnLaquuxDTNZAzhFpfcAriAFW7gxF85yMSlGy2G4iPAqXo0zBaAthZAGXjBSD1XoZBze4Ph87msqTbWt7QDR0gKf50OUNyyBaFktxOVCz4gg579yFrFLrc4Vxn3i3pmJaMgbZA3vASrUVP7a8QdbTujW8egJlpsZAJoFZBz8UooFNo01Y0S06WMtdajiw4G3CvKVzMWEjmtXC0fPdmvIgTZAtaC`,
...
exports.receiveMessage = async (req, res) => {
  ...
  if (
    ...
    msg.interactive.button_reply?.id?.startsWith("R")
  ) {
  ...
  if (msg.type === "location") {
    ...
    if (!nearby.length)
      return sendText(
        phone,
        "😕 Sorry, we are not delivering to your location yet."
      );
    ...
    return sendButtons(phone, "✅ We deliver in your area!", [
      ...
      {
        type: "reply",
        reply: { id: "ORDER_GROCERY", title: "🍽 Order Grocery" },
      },
      {
        type: "reply",
  ...
  if (
    ...
    if (!user.location?.lat || !user.location?.lng) {
      return sendText(
        phone,
        "📍 Please share location first. Type *hi* again."
      );
    }
    ...
    return sendList(phone, "🛒 Select a grocery store near you:", rows);
  }
  ...
  if (
    ...
    user.chatState = "SELECT_RESTAURANT";
    await user.save();
    await updateCache(user);
    ...
    const nearby = restaurants.filter((r) => {
      const loc = r.merchantId?.address?.location;
      if (!loc) return false;
      return (
        distanceKM(user.location.lat, user.location.lng, loc.lat, loc.lng) <= 5
      );
    });

    if (!nearby.length) {
      return sendText(phone, "😕 No restaurants available near you.");
    }
    ...
    const rows = [
      {
        ...
        description: "Show items from all nearby restaurants",
      },
      ...nearby.map((r) => ({
        id: `REST_${r._id}`,
        title: r.merchantId.storeName.slice(0, 24),
        description: r.merchantId.address.city || "Nearby",
      })),
    ];

    return sendList(phone, "🏪 Select a restaurant", rows);
  }

  if (
    msg.type === "interactive" &&
    msg.interactive.list_reply?.id?.startsWith("REST_")
  ) {
    const restId = msg.interactive.list_reply.id;

    user.selectedRestaurant =
      restId === "REST_ALL" ? "ALL" : restId.replace("REST_", "");
  ...
  if (
    msg.type === "interactive" &&
    msg.interactive.button_reply?.id?.startsWith("TYPE_")
  ) {
    const foodType =
      msg.interactive.button_reply.id === "TYPE_VEG" ? "VEG" : "NON-VEG";
  ...
  if (
    msg.type === "interactive" &&
    msg.interactive.list_reply?.id?.startsWith("CAT_")
  ) {
  ...
  //     description: `₹${r.menuItems.price} · ⭐${r.menuItems.rating?.toFixed(
  //       1
  //     )}`, // safe
  //   }));
  ...
  if (
    ...
    if (!nearbyAgents.length) {
      return sendText(
        phone,
        "⏳ No nearby delivery agents available right now."
      );
    }
...
`;
    ...
    try {
      ...
      return sendText(
        phone,
        "❌ I couldn't understand that. Please type again."
      );
    }
    ...
    for (const obj of parsedItems) {
      ...
      const match = store.items.find((i) => i.name.toLowerCase() === itemName);
  ...
  if (
    msg.type === "interactive" &&
    msg.interactive.button_reply?.id === "ORDER_MEDICINE"
  ) {
    user.chatState = "ASK_MEDICINE_INPUT_TYPE";
    await user.save();
    await updateCache(user);

    return sendButtons(phone, "💊 How would you like to order medicine?", [
      {
        type: "reply",
        reply: { id: "MED_UPLOAD", title: "📸Upload Prescription" },
      },
      {
        type: "reply",
        reply: { id: "MED_TEXT", title: "✍️Type Medicine Names" },
      },
    ]);
  }
  ...
  if (
    msg.type === "interactive" &&
    msg.interactive.button_reply?.id === "MED_UPLOAD"
  ) {
    ...
    return sendText(
      phone,
      "📸 Please upload a clear photo of your prescription now."
    );
  }
  ...
  if (
    msg.type === "interactive" &&
    msg.interactive.button_reply?.id === "MED_TEXT"
  ) {
    ...
    return sendText(
      phone,
      "💊 Please type the medicines, one per line. E.g.\nParacetamol 500mg 10 tablets\nCough syrup 100ml"
    );
  }
  ...
  if (
    msg.type === "interactive" &&
    msg.interactive.button_reply?.id?.startsWith("SHOP_ACCEPT_")
  ) {
    ...
    return sendText(
      msg.from,
      `✅ Accepted. Please reply with the total price for Order ${orderId} as a number, e.g. 350`
    );
  }

  if (
    msg.type === "interactive" &&
    msg.interactive.button_reply?.id?.startsWith("SHOP_REJECT_")
  ) {
    const [, orderId, shopId] = msg.interactive.button_reply.id.split("_");
  ...
  if (
    msg.type === "interactive" &&
    msg.interactive.list_reply?.id?.startsWith("OFFER_")
  ) {
    ...
    const order = await MedicineOrder.findById(orderId).populate(
      "offers.pharmacyId"
    );
    ...
    await sendText(
      offer.pharmacyPhone,
      `✅ Customer selected your price ₹${offer.price} for Order ${order._id}. Please prepare medicines.`
    );
    ...
    return sendButtons(
      phone,
      `You selected ${offer.pharmacyName} · ₹${offer.price}\nChoose payment:`,
      [
        {
          type: "reply",
          reply: { id: "MED_COD", title: "💵 Cash on Delivery" },
        },
  ...
  if (msg.type === "text") {
    ...
    const shop = await Merchant.findOne({
      $or: [{ phone: senderPhone }, { "merchantId.phone": senderPhone }],
    });
    if (shop && shop.businessType === "PHARMACY") {
      ...
      if (!m) {
        ...
        return sendText(
          msg.from,
          "Please reply with the total price as a number. Example: 350"
        );
      }
      ...
      if (!order) {
        return sendText(
          msg.from,
          "No pending medicine request found to submit price for."
        );
      }
      ...
      await sendText(
        msg.from,
        `✅ Price received: ₹${price} for Order ${order._id}`
      );
      ...
      await sendList(
        customer.phone,
        `💊 Offers received for your medicine request (Order ${order._id}):`,
        rows
      );
      return;
    }
  ...
  if (
    msg.type === "interactive" &&
    msg.interactive.button_reply?.id === "MED_COD"
  ) {
    // user selected COD after picking offer
    if (!user.tempMedicineOrderId)
      return sendText(phone, "No active medicine order found.");
  ...
  if (
    msg.type === "interactive" &&
    msg.interactive.button_reply?.id === "MED_UPI"
  ) {
    if (!user.tempMedicineOrderId)
      return sendText(phone, "No active medicine order found.");
    const medOrder = await MedicineOrder.findById(user.tempMedicineOrderId);
...
async function processGroceryOrder(user, phone) {
  const store = await GroceryStore.findById(user.tempGroceryStore).populate(
    "merchantId"
  );
  if (!store) return sendText(phone, "⚠ Store not found.");
...
async function processMedicineRequest(user, phone) {
  ...
  if (!user.location?.lat || !user.location?.lng) {
    ...
    return sendText(
      phone,
      "📍 Please share your location first. Type hi to restart."
    );
  }
  ...
  for (const shop of notifyShops) {
    ...
    await sendButtons(shopPhone, body, [
      {
        type: "reply",
        reply: {
          id: `SHOP_ACCEPT_${order._id}_${shop._id}`,
          title: "Accept & Enter Price",
        },
      },
      {
        type: "reply",
        reply: { id: `SHOP_REJECT_${order._id}_${shop._id}`, title: "Reject" },
      },
    ]);
  }
  ...
  await sendText(
    phone,
    "⏳ Request sent to nearby pharmacies. We'll notify you when offers arrive."
  );
  return;
}
